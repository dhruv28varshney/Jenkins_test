...
static int devnum = 0;
static int Major = 259;         /* Major number assigned to our device driver */
...
{
        //int bytes_read = 0;
        ssize_t len = min((ssize_t)num_bytes - *offset, length);
        if (len <= 0) return 0;

        if (offset == NULL) return -EINVAL;
        if (*offset >= num_bytes) return 0;

        /* read data from msg to user buffer */
        if (copy_to_user(buffer, msg + *offset, len))
            return -EFAULT;

        *offset += len;
        printk(KERN_ALERT "Inside the %s function using copy to user\n", __FUNCTION__);
        return len;
/*
...
ssize_t my_write(struct file *pfile, const char __user *buffer, size_t length, loff_t *offset)
{
        ...
        /* read data from user_buffer to msg */
        if (copy_from_user( msg + *offset, buffer, len))
            return -EFAULT;

        *offset += len;

        printk(KERN_ALERT "Inside the %s function using copy from user \n", __FUNCTION__);
        printk(KERN_INFO "(%s)\n",msg);
        return len;
}
...
{
        printk(KERN_ALERT "Inside the %s function in char_driv_fops \n", __FUNCTION__);
